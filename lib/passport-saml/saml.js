...
SAML.prototype.requestToUrl = function (request, options, operation, callback) {
  var self = this;
  zlib.deflateRaw(request, function(err, buffer) {
    ...
    if (options.RelayState) {
      ...
      target += '&' + querystring.stringify(relayState);
    }
...
SAML.prototype.getAuthorizeUrl = function (req, options, callback) {
  ...
  this.requestToUrl(request, options, 'authorize', callback);
};
...
SAML.prototype.validateResponse = function (samlResponse, callback) {
  ...
  parser.parseString(xml, function (err, doc) {
    ...
    var response = self.getElement(doc, 'saml2p:Response');
    if (response) {
      var assertion = self.getElement(response, 'saml2:Assertion');
      if (!assertion) {
        return callback(new Error('Missing SAML assertion'), null, false);
      }

      profile = {};
      var issuer = self.getElement(assertion[0], 'saml2:Issuer');
      if (issuer) {
        profile.issuer = issuer[0];
      }

      var subject = self.getElement(assertion[0], 'saml2:Subject');
      if (subject) {
        var nameID = self.getElement(subject[0], 'saml2p:NameID');
        if (nameID) {
      ...
      var attributeStatement = self.getElement(assertion[0], 'saml2:AttributeStatement');
      if (!attributeStatement) {
        return callback(new Error('Missing AttributeStatement'), null, false);
      }

      var attributes = self.getElement(attributeStatement[0], 'saml2:Attribute');

      if (attributes) {
        attributes.forEach(function (attribute) {
          var value = self.getElement(attribute, 'saml2:AttributeValue');
